...
top_am <- dat[order(dat$am_t_f, decreasing = TRUE),] %>% 
  head(n = temp_n)
tail_am<- dat[order(dat$am_t_f),] %>% 
  head(n = temp_n)
am_high_low <- top_am %>% 
  rbind(tail_am)
...
m_am <- leaflet() %>% 
  ...
  addMapPane('am_high_low', zIndex = 410) %>%
  ...
  addPolygons(data= am_high_low, color = "blue",
                    fill = FALSE,
                    weight = 1.5,
                    group = 'Morning Highest & Lowest Temps',
                    options = pathOptions(pane = 'am_high_low')) %>%
  ...
  addLayersControl(overlayGroups = c("Morning Highest & Lowest Temps"),
                         options = layersControlOptions(collapsed = FALSE),
                         position = "bottomleft")

af_title <- h4("Afternoon temperatures", class = "title")

top_af <- dat[order(dat$af_t_f, decreasing = TRUE),] %>% 
  head(n = temp_n)
tail_af<- dat[order(dat$af_t_f),] %>% 
  head(n = temp_n)
af_high_low <- top_af %>% 
  rbind(tail_af)
...
m_af <- leaflet() %>% 
  addProviderTiles("CartoDB.Voyager") %>% 
  fitBounds(bbox[1], bbox[2], bbox[3], bbox[4]) %>%
  addMapPane('af_high_low', zIndex = 410) %>%
  addPolygons(data = dat,
  ...
  addPolygons(data= af_high_low, color = "blue",
                    fill = FALSE,
                    weight = 1.5,
                    group = 'Afternoon Highest & Lowest Temps',
                    options = pathOptions(pane = 'af_high_low')) %>%
  leaflet::addLegend("bottomright", 
                     pal = pal_all_rev, 
                     values = temp_range$temp, 
                     opacity = 0.7, 
  ...
  addLayersControl(overlayGroups = c("Afternoon Highest & Lowest Temps"),
                         options = layersControlOptions(collapsed = FALSE),
                         position = "bottomleft")

pm_title <- h4("Evening temperatures", class = "title")

top_pm <- dat[order(dat$pm_t_f, decreasing = TRUE),] %>% 
  head(n = temp_n)
tail_pm<- dat[order(dat$pm_t_f),] %>% 
  head(n = temp_n)
pm_high_low <- top_pm %>% 
  rbind(tail_pm)
...
m_pm <- leaflet() %>% 
  addProviderTiles("CartoDB.Voyager") %>% 
  fitBounds(bbox[1], bbox[2], bbox[3], bbox[4]) %>%
  addMapPane('pm_high_low', zIndex = 410) %>%
  addPolygons(data = dat,
  ...
  addPolygons(data= pm_high_low, color = "blue",
                    fill = FALSE,
                    weight = 1.5,
                    group = 'Evening Highest & Lowest Temps',
                    options = pathOptions(pane = 'pm_high_low')) %>%
  leaflet::addLegend("bottomright", 
                     pal = pal_all_rev, 
                     values = temp_range$temp, 
                     opacity = 0.7, 
  ...
  addLayersControl(overlayGroups = c("Evening Highest & Lowest Temps"),
                         options = layersControlOptions(collapsed = FALSE),
                         position = "bottomleft")
...
# ggplot(am_high_low, aes(x = reorder(NAME, -am_t_f), y = am_t_f, fill = am_t_f)) + 
...
# dat$Blacktile <- ntile(dat$percBlack, 5)
# dat$Blacktile <- as.factor(dat$Blacktile)
# 
# dat$Whitetile <- ntile(dat$percWhite, 5)
# dat$Whitetile <- as.factor(dat$Whitetile)
# 
# dat$Hisptile <- ntile(dat$percHisp, 5)
...
#   scale_fill_manual(values = c('#6CC296', '#6CC296', '#6CC296', "#6CC296", "#6CC296")) +
...
# Percent Tree Canopy and Percent Black Residents

b_tree <- ggplot(bdat_tree, aes(x = bucket, y = Tree,
                            fill = bucket, label = Tree,
                                  text = paste0('Mean', ': ', round(Tree, digits = 2)))) +
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = c('#6CC296', '#6CC296', '#6CC296', "#6CC296", "#6CC296")) +
  labs(x = bdat_tree$bucket,
       y = bdat_tree$Tree) +
        labs(x = "Percent Black Residents Rank",
             y = "Average Tree Canopy") +
        theme_minimal() + ggtitle("Average Tree Canopy and Percent Black Residents") + geom_hline(yintercept = mean(dat$Histogram, na.rm = T), linetype = 'dashed', color = 'black', size = 1) + annotate("text", x = 4, y = (mean(dat$Histogram, na.rm = T) + 2), label = paste0("Overall Average: ", round(mean(dat$Histogram, na.rm = T), 2)))

# ggplotly(b_tree, tooltip = c('text')) %>%
#         layout(showlegend = FALSE, yaxis = list(side = "right", fixedrange = T), xaxis = list(fixedrange = T))
...
#                                   text = paste0('Mean', ': ', round(Tree, digits = 2)))) +
#   geom_bar(stat = 'identity') +
#   scale_fill_manual(values = c('#dfb0d6', '#a5add3', '#569ab9', "cornflowerblue","#6CC296")) +
...
# Percent Tree Canopy and Percent White Residents

w_test <- ggplot(wdat_test, aes(x = bucket, y = Tree,
                            fill = bucket, label = Tree,
                                  text = paste0('Mean', ': ', round(Tree, digits = 2)))) +
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = c('#6CC296', '#6CC296', '#6CC296', "#6CC296", "#6CC296")) +
  labs(x = wdat_test$bucket,
       y = wdat_test$Tree) +
        labs(x = "Percent White Residents Rank",
             y = "Average Tree Canopy") +
        theme_minimal() + ggtitle("Average Tree Canopy and Percent White Residents") + geom_hline(yintercept = mean(dat$Histogram, na.rm = T), linetype = 'dashed', color = 'black', size = 1) + annotate("text", x = 4, y = (mean(dat$Histogram, na.rm = T) + 2), label = paste0("Overall Average: ", round(mean(dat$Histogram, na.rm = T), 2)))

# ggplotly(w_test, tooltip = c('text')) %>%
#         layout(showlegend = FALSE, yaxis = list(side = "right", fixedrange = T), xaxis = list(fixedrange = T))
...
# Percent Tree Canopy and Percent Hispanic Residents

h_test <- ggplot(hdat_test, aes(x = bucket, y = Tree,
                            fill = bucket, label = Tree,
                                  text = paste0('Mean', ': ', round(Tree, digits = 2)))) +
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = c('#6CC296', '#6CC296', '#6CC296', "#6CC296", "#6CC296")) +
  labs(x = hdat_test$bucket,
       y = hdat_test$Tree) +
        labs(x = "Percent Hispanic Residents Rank",
             y = "Average Tree Canopy") +
        theme_minimal() + ggtitle("Average Tree Canopy and Percent Hispanic Residents") + geom_hline(yintercept = mean(dat$Histogram, na.rm = T), linetype = 'dashed', color = 'black', size = 1) + annotate("text", x = 4, y = (mean(dat$Histogram, na.rm = T) + 2), label = paste0("Overall Average: ", round(mean(dat$Histogram, na.rm = T), 2)))

# ggplotly(h_test, tooltip = c('text')) %>%
#         layout(showlegend = FALSE, yaxis = list(side = "right", fixedrange = T), xaxis = list(fixedrange = T))

# subplot(b_tree, w_test, h_test, nrows = 1, margin = 0.04) %>%
#         layout(showlegend = FALSE, yaxis = list(side = "right", fixedrange = T), xaxis = list(fixedrange = T))

# gplot <- ggarrange(b_tree, w_test, h_test, nrow = 1, common.legend = TRUE)

#doesnt work
# ggplotly(gplot, tooltip = c('text')) %>%
#         layout(showlegend = FALSE, yaxis = list(side = "right", fixedrange = T), xaxis = list(fixedrange = T))
```
...
bdat_heat <- dat %>% 
  ...
  group_by(bucket) %>% 
  # mutate(heat = median(pm_t_f, na.rm = T)) %>% 
  mutate(heat = mean(pm_t_f, na.rm = T)) %>%
  slice(1) %>% 
  mutate(mean = mean(dat$pm_t_f, na.rm = T),
         median = median(dat$pm_t_f, na.rm = T),
         med_dif = (heat - median(dat$pm_t_f, na.rm = T)),
        difference = (heat - mean(dat$pm_t_f, na.rm = T)),
        perc_of_mean = ((heat - mean(dat$pm_t_f, na.rm = T)) / mean(dat$pm_t_f, na.rm = T) * 100))

b_heat <- ggplot(
  bdat_heat, aes(x = bucket, y = difference, fill = difference, label = difference, text = paste0('Difference', ': ', round(difference, digits = 2), "°"))
  ) +
  geom_bar(stat = 'summary') +
  ...
  # labs(x = bdat_heat$bucket,
  #      y = bdat_heat$heat) +
  #       labs(x = "Percent Black Residents Rank",
  #            y = "Average Tree Canopy") +
        theme_minimal() +
  geom_hline(yintercept = 0, linetype = 'dashed', color = 'black', size = 1) +
  annotate("text", x = 4, y = .1, label = paste0("Mean Evening Temperature: ", round(mean(dat$pm_t_f, na.rm = T), 0), "°"))

# ggplotly(b_heat, tooltip = c('text')) %>%
#         layout(showlegend = FALSE, yaxis = list(fixedrange = T), xaxis = list(fixedrange = T))
...
# w_heat <- ggplot(wdat_heat, aes(x = bucket, y = difference, 
#                                 fill = difference,
#                                 label = difference,
#                                   text = paste0('Difference', ': ', round(difference, digits = 2), "°"))) +
#   geom_bar(stat = 'summary') +
...
#         theme_minimal() +
#   geom_hline(yintercept = 0, linetype = 'dashed', color = 'black', size = 1) +
#   annotate("text", x = 4, y = .1, label = paste0("Average Evening Temperature: ", round(mean(dat$pm_t_f, na.rm = T), 0), "°"))
# 
# ggplotly(w_heat, tooltip = c('text')) %>%
#         layout(showlegend = FALSE, yaxis = list(fixedrange = T), xaxis = list(fixedrange = T))
...
gg1_heat <- ggplotly((
    ggplot(bdat_heat, 
        ...
        annotate("text", 
                 x = 3.5, 
                 y = -0.1, 
                 label = paste0("Average Evening Temperature: ", round(mean(dat$pm_t_f, na.rm = T), 0), "°"),
                 size = 3.5
                 ) +
        theme_minimal()),
     tooltip = c('text')
  ...
  layout(showlegend = FALSE, 
           yaxis = list(fixedrange = T, title=list(text="Difference from Average Evening Temp",
                      standoff= 10)), 
  ...
  subplot(gg1_heat,gg2_heat, gg3_heat, nrows = 1, 
  ...
  gg1 <- ggplotly((
    ggplot(bdat_tree, 
             aes(x = bucket, 
                 y = Tree,
                 label = Tree, 
                 text = paste0('Mean', ': ', round(Tree, digits = 0))
                 )
             ) + 
