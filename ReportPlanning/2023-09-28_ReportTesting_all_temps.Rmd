---
title: "Map Testing & Report Planning - Full Day Temp Scale"
...
temp_select <- temp_range$temp

pal <- colorNumeric("Spectral", domain = temp_select, reverse = T)
pal_rev <- colorNumeric("Spectral", domain = temp_select, reverse = F)
...
leaflet() %>% 
  ...
  leaflet::addLegend("bottomright", pal = pal_rev, values = temp_select, opacity = 0.7, labFormat = labelFormat(transform = function(x) sort(x, decreasing = TRUE)))
...
temp_limits <- c(min(dat$am_t_f), max(dat$af_t_f))
